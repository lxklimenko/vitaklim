...
if (!MAX_TOKEN) {
    console.error("ОШИБКА: Переменная MAX_BOT_TOKEN не найдена!");
    process.exit(1);
}
...
bot.action('action_back', async (ctx: any) => {
  ...
  switch (currentState) {
    case "awaiting_prompt": { await sendFormatSelection(ctx, maxUserId, modelName || MODELS.NANO2); break; }
    case "choosing_format": { await sendModelSelection(ctx, maxUserId); break; }
    case "awaiting_photo": { await sendPhotoFormatSelection(ctx, maxUserId, modelName || MODELS.NANO2); break; }
    case "choosing_photo_format": { await sendPhotoModelSelection(ctx, maxUserId); break; }
...
bot.action('action_start_over', async (ctx: any) => {
  ...
  const { data: profile } = await supabaseAdmin.from("profiles").select("bot_state").eq("max_user_id", maxUserId).maybeSingle();
...
bot.on('message_created', async (ctx: any) => {
  ...
  const { data: profile } = await supabaseAdmin.from("profiles").select("*").eq("max_user_id", maxUserId).limit(1).maybeSingle();

  if (!profile) return;
  const currentState = profile.bot_state || "idle";

  if (text === "💰 Баланс" || text === "action_balance") {
    const keyboard = Keyboard.inlineKeyboard([[Keyboard.button.callback("💳 Пополнить баланс", "action_start_payment")]]);
    await ctx.reply(`💰 *Ваш баланс:* ${profile.balance} 🍌\n\nЗдесь вы можете пополнить счет.`, { format: 'markdown', attachments: [keyboard] });
    return;
  }
  if (text === "📜 История" || text === "action_history") { await ctx.reply("📂 История генераций скоро появится!", { format: 'markdown' }); return; }
  if (text === "❓ Помощь" || text === "action_help") { await ctx.reply("🚀 *Помощь*\n\nВсё очень просто: жми на кнопки!", { format: 'markdown' }); return; }
  if (text === "⚙️ Настройки" || text === "action_settings") { await ctx.reply("⚙️ Настройки в разработке.", { format: 'markdown' }); return; }
  if (text === "🎨 Создать картинку") { await sendModelSelection(ctx, maxUserId); return; }
  if (text === "🖼 Сгрен. по фото") { await sendPhotoModelSelection(ctx, maxUserId); return; }

  if (currentState === "awaiting_broadcast_text") {
    if (maxUserId !== ADMIN_MAX_ID) { await updateBotState(maxUserId, "idle"); await sendMaxMainMenu(ctx); return; }

    const broadcastText = text;
    if (!broadcastText) { await ctx.reply("❌ Отправьте текст рассылки (не пустое сообщение)."); return; }

    await ctx.reply("📢 Начинаю рассылку... Это может занять некоторое время.");

    const { data: users, error } = await supabaseAdmin.from('profiles').select('max_user_id').not('max_user_id', 'is', null);
    ...
    for (const user of users) {
      try {
        await bot.api.sendMessageToUser(Number(user.max_user_id), broadcastText, { format: 'markdown' });
        successCount++;
  ...
  if (currentState === "awaiting_payment_email") {
    ...
    await supabaseAdmin.from("profiles").update({ bot_state: "awaiting_payment_amount", bot_selected_model: email }).eq("max_user_id", maxUserId);

    const keyboard = Keyboard.inlineKeyboard([[Keyboard.button.callback("⬅️ Назад", "action_back")]]);
    await ctx.reply("Введите сумму пополнения в рублях ✍️\n(Например: 100, 200 или 500)", { attachments: [keyboard] });
    return;
  }
  ...
  if (currentState === "awaiting_payment_amount") {
    ...
    try {
      ...
      const paymentResponse = await fetch(`${SITE_URL}/api/payment`, {
        ...
        body: JSON.stringify({ amount, telegramUserId: profile.telegram_id || profile.max_user_id, email: userEmail, from: 'max' }),
      });
      ...
      if (!paymentResponse.ok || !paymentData.confirmationUrl) {
        ...
        await ctx.reply(`💳 **Пополнение на ${amount} ₽**\n\n🔗 [Нажмите сюда, чтобы перейти к оплате](${paymentData.confirmationUrl})`, { format: 'markdown' });
      }
    } catch (err) {
  ...
  if (currentState === "awaiting_photo") {
    ...
    try {
      ...
      if (!fileUrl) { await ctx.reply("Ошибка: не удалось прочитать ссылку на файл."); return; }

      const currentUrls = profile.bot_reference_url || [];
      const updatedUrls = [...currentUrls, fileUrl];

      await supabaseAdmin.from("profiles").update({ bot_state: "awaiting_photo_prompt", bot_reference_url: updatedUrls }).eq("max_user_id", maxUserId);

      const keyboard = Keyboard.inlineKeyboard([[Keyboard.button.callback("⬅️ Назад", "action_back")]]);
      await ctx.reply("📸 Фото принято!\n\nТеперь **напишите описание** (промпт) для генерации 🎨", { format: 'markdown', attachments: [keyboard] });
    } catch (error: any) {
  ...
  if (currentState === "awaiting_photo_prompt") {
    if (!text) { await ctx.reply("Пожалуйста, напишите текстовое описание для вашего фото ✍️"); return; }
    await handlePhotoGeneration(ctx, profile, text);
    return;
  }

  if (currentState === "awaiting_prompt") {
    if (!text) { await ctx.reply("Пожалуйста, напишите, что нужно создать ✍️"); return; }
...
async function handleTextGeneration(ctx: any, profile: any, prompt: string) {
  ...
  try {
    const result = await generateImageCore({ userId: profile.id, prompt, modelId, aspectRatio: formatFromDb || "1:1", supabase: supabaseAdmin, imageBuffers: undefined });
    ...
    await ctx.reply(`📁 **Оригинал в максимальном качестве:**\n🔗 [Скачать HD оригинал](${downloadUrl})`, { format: 'markdown', attachments: [keyboard] });
...
async function handlePhotoGeneration(ctx: any, profile: any, prompt: string) {
  ...
  try {
    ...
    const result = await generateImageCore({ userId: profile.id, prompt, modelId, aspectRatio: formatFromDb || "1:1", supabase: supabaseAdmin, imageBuffers: [userImageBuffer] });
...
// ==================== ЗАПУСК POLLING ====================
console.log("🚀 MAX бот запускается в режиме polling...");
bot.start().catch((err) => {
